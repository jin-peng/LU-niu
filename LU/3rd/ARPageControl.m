...
#import "ARPageControl.h"

#define DEFAULT_INDICATOR_WIDTH 6.0f
#define DEFAULT_INDICATOR_MARGIN 6.0f

@interface ARPageControl ()
@end

@implementation ARPageControl
{
@private
...
- (void)_initialize
{
	...
	_alignment = ARPageControlAlignmentCenter;
	_verticalAlignment = ARPageControlVerticalAlignmentMiddle;
	...
	for (NSUInteger i = 0; i < _numberOfPages; i++) {
		...
		if (i == _displayedPage) {
			...
			if (nil == image) {
                ...
                xOffset -= image.size.width/4;
			}
		} else {
			fillColor = _pageIndicatorTintColor ? _pageIndicatorTintColor : [[UIColor grayColor] colorWithAlphaComponent:0.3f];
			//image = _pageImages[@(i)];
			if (nil == image) {
				image = _pageIndicatorImage;
                xOffset += image.size.width/2;
			}
		}
		[fillColor set];
...
- (CGFloat)_leftOffset
{
	...
	CGFloat left = 0.0f;
	switch (_alignment) {
		case ARPageControlAlignmentCenter:
			left = CGRectGetMidX(rect) - (size.width / 2.0f);
			break;
		case ARPageControlAlignmentRight:
			left = CGRectGetMaxX(rect) - size.width;
			break;
        case ARPageControlAlignmentLeft:
            left = CGRectGetMaxX(rect) - _MainScreen_Width + 7;
            break;
		default:
			break;
	}
...
- (CGFloat)_topOffset
{
	...
	switch (_verticalAlignment) {
		case ARPageControlVerticalAlignmentMiddle:
			top = CGRectGetMidY(rect) - (_measuredIndicatorHeight / 2.0f);
			break;
		case ARPageControlVerticalAlignmentBottom:
...
{
    [self setCurrentPageIndicatorImage:_currentPageIndicatorImage];
    [self setPageIndicatorImage:_pageIndicatorImage];
