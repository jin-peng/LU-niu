...
@interface CHBuy5kViewController ()<UITextFieldDelegate> {
    CGFloat buyNum;
    CGFloat buyMoney;
    CGFloat minNum;
    CGFloat zhiyingMax;
...
@implementation CHBuy5kViewController
...
- (void)refrshUI{
    ...
    buyMoney = 0;
    minNum = 0;
    while (buyMoney<[_userPlan[@"cc_max"] floatValue]) {
        minNum++;
        buyMoney = minNum * 100 * [_strckPriceDic[@"price"] floatValue];
    }
    minNum--;
    if (minNum <= 0) {
        [CNManager showWindowAlert:@"股价过高,禁止买入"];
    }
    [self refreshPrice];
...
- (void)refreshPrice{
    ...
    _baozhengjin.text = [NSString stringWithFormat:@"履约保证金        1.0元"];
    zongji = [NSString stringWithFormat: @"%.2f",(buyMoney/10000.)*[_userPlan[@"gl_money"] floatValue] + buyMoney/[_userPlan[@"pry"]floatValue]];
    NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat: @"总计     %.2f  元",((buyMoney/10000.)*[_userPlan[@"gl_money"] floatValue] + 1.)]];
...
//    [str1 addAttribute:NSForegroundColorAttributeName value:[UIColor redColor] range:NSMakeRange(15 + fu.length, dy.length)];
    _fudongshuoming.text = @"温馨提示：所有活动买入股票会在第二天14:48被自动平仓。";
}
...
- (void)refreshAll{
    ...
    [CHGetStockNum getUserPlanSuccess:^(NSDictionary * _Nonnull planDic) {
...
//        _setZY.layer.cornerRadius = 3.;
        ...
        _setZY.userInteractionEnabled = NO;
...
//        _setZS.layer.cornerRadius = 3.;
        ...
        UILabel *lb =[JPMyControl createLabelWithFrame:CGRectMake(-15, 0, 15, _setZY.height) Font:18 Text:@"-"];
        lb.textColor = [UIColor redColor];
        _setZS.userInteractionEnabled = NO;
...
- (UITextField *)stockNum{
    if (!_stockNum) {
        ...
        _stockNum.userInteractionEnabled = NO;
    }
    return _stockNum;
}
...
- (void)helpBtnClick{
    CGFloat x = [_userPlan[@"cc_max"] floatValue];
    CGFloat y = x/100.;
    UIAlertView * al = [[UIAlertView alloc]initWithTitle:@"提示" message:[NSString stringWithFormat: @"活动买入股票数量为小于%.0f元总额的最大数量，单价大于%.2f元的股票不能买入。",x,y] delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
    [al show];
}
...
- (void)buyBtnClick{
    NSInteger x = [_stockNum.text integerValue]/100;
    if (x<=0) {
        [CNManager showWindowAlert:@"不能买入0股"];
        return;
    }
    if (x < minNum) {
        buyNum = minNum;
...
//    [dataDict setObject:_setZS.text forKey:@"stop_loss"];
    ...
    NSString *URLString =[NSString stringWithFormat:@"http://%@/Api/activity/activityOrder",
